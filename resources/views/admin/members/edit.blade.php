...
@section('content')
<form id="memberForm" method="POST" action="{{ route('admin.members.update', $member) }}" enctype="multipart/form-data" class="space-y-4 sm:space-y-5" novalidate>
    @csrf @method('PUT')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5">
        ...
        <div class="lg:col-span-2 space-y-4 sm:space-y-5">
            ...
            <div class="bg-white rounded-xl sm:rounded-2xl border border-slate-200 p-4 sm:p-5 space-y-3 sm:space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="nameInput" value="{{ old('name', $member->name) }}"
                ...
                <div>
                    ...
                    <input type="text" name="position" id="positionInput" value="{{ old('position', $member->position) }}"
                ...
                <div>
                    ...
                    <input type="text" name="division" id="divisionInput" value="{{ old('division', $member->division) }}"
        ...
        <div class="space-y-4 sm:space-y-5">
            ...
            <div class="bg-white rounded-xl sm:rounded-2xl border border-slate-200 p-4 sm:p-5">
                <h2 class="font-semibold text-slate-800 text-xs sm:text-sm mb-1">Foto</h2>
                <p class="text-[10px] sm:text-xs text-slate-400 mb-3">
                    @if($member->photo) Foto saat ini ditampilkan. Upload baru untuk menggantinya.
                    @else Foto wajib diupload. @endif
                </p>
                <div x-data="imagePreview('{{ $member->photo ? Storage::url($member->photo) : '' }}')" class="space-y-2">
...
<script>
    ...
    const hasExistingPhoto = {{ $member->photo ? 'true' : 'false' }};

    document.getElementById('memberForm').addEventListener('submit', function(e) {
        let valid = true;
        const hasNewPhoto = document.getElementById('photoInput').files.length > 0;
        ...
        const checks = [
            ...
            { id: 'photoInput',    errId: 'photoError',    boxId: 'photoBox', test: () => hasExistingPhoto || hasNewPhoto,                              msg: 'Foto wajib diupload.' },
        ];
