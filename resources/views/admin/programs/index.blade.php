...
<div class="bg-white rounded-xl sm:rounded-2xl border border-slate-200 overflow-hidden">
        ...
        <table class="w-full text-xs sm:text-sm">
            <thead>
                <tr class="border-b border-slate-200 bg-slate-50 text-left">
                    <th class="px-3 sm:px-5 py-3 font-medium text-slate-500 w-14 sm:w-20">Gambar</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    <td class="px-3 sm:px-5 py-2 sm:py-3">
                        @if($program->image)
                            <img src="{{ Storage::url($program->image) }}" class="h-8 w-12 sm:h-10 sm:w-16 rounded-lg object-cover" alt="">
                        @else
                            <div class="h-8 w-12 sm:h-10 sm:w-16 rounded-lg bg-slate-100"></div>
                        @endif
                    </td>
                    <td class="px-2 sm:px-5 py-2 sm:py-3 max-w-0">
                        <p class="font-medium text-slate-800 truncate">{{ $program->title }}</p>
                    </td>
                    ...
                    <td class="px-2 sm:px-4 py-2 sm:py-3 w-24 sm:w-32">
                        <div class="flex items-center justify-end gap-1 sm:gap-2">
                            ...
                            <button type="button"
                                onclick="confirmDelete('{{ route('admin.programs.destroy', $program) }}', '{{ addslashes($program->title) }}')"
                ...
                <tr>
                    <td colspan="4" class="px-5 py-10 sm:py-12 text-center text-xs sm:text-sm text-slate-400">
                        Belum ada program. <a href="{{ route('admin.programs.create') }}" class="text-sky-600 hover:underline">Tambah sekarang</a>
                    </td>
                </tr>
                @endforelse
            </tbody>
    ...
    <div class="border-t border-slate-100 px-3 sm:px-5 py-3">{{ $programs->links() }}</div>
    @endif
</div>
