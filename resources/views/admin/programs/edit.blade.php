...
@section('content')
<form id="programForm" method="POST" action="{{ route('admin.programs.update', $program) }}" enctype="multipart/form-data" class="space-y-4 sm:space-y-5" novalidate>
    @csrf @method('PUT')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5">
        ...
        <div class="lg:col-span-2 space-y-4 sm:space-y-5">
            ...
            <div class="bg-white rounded-xl sm:rounded-2xl border border-slate-200 p-4 sm:p-5 space-y-3 sm:space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="titleInput" value="{{ old('title', $program->title) }}"
                ...
                <div>
                    ...
                    <div id="quillEditor">{!! old('description', $program->description) !!}</div>
        ...
        <div class="space-y-4 sm:space-y-5">
            ...
            <div class="bg-white rounded-xl sm:rounded-2xl border border-slate-200 p-4 sm:p-5">
                <h2 class="font-semibold text-slate-800 text-xs sm:text-sm mb-1">Gambar Program</h2>
                <p class="text-[10px] sm:text-xs text-slate-400 mb-3">
                    @if($program->image) Gambar saat ini ditampilkan. Upload baru untuk menggantinya.
                    @else Gambar wajib diupload. @endif
                </p>
                <div x-data="imagePreview('{{ $program->image ? Storage::url($program->image) : '' }}')" class="space-y-2">
...
<script>
    ...
    const hasExistingImage = {{ $program->image ? 'true' : 'false' }};

    document.getElementById('programForm').addEventListener('submit', function(e) {
        let valid = true;
        const desc = quill.getText().trim();
        document.getElementById('descriptionInput').value = quill.root.innerHTML;
        const hasNewImage = document.getElementById('imageInput').files.length > 0;
        ...
        const checks = [
            ...
            { id: 'imageInput', errId: 'imageError', boxId: 'imageBox', test: () => hasExistingImage || hasNewImage,                          msg: 'Gambar wajib diupload.' },
        ];
