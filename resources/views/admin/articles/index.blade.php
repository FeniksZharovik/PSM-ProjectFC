...
<div class="bg-white rounded-xl sm:rounded-2xl border border-slate-200 overflow-hidden">
        ...
        <table class="w-full text-xs sm:text-sm">
            <thead>
                <tr class="border-b border-slate-200 bg-slate-50 text-left">
                    <th class="px-3 sm:px-5 py-3 font-medium text-slate-500 w-10 sm:w-14">Thumb</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    <td class="px-3 sm:px-5 py-2 sm:py-3">
                        @if($article->thumbnail)
                            <img src="{{ Storage::url($article->thumbnail) }}" class="h-8 w-8 sm:h-10 sm:w-10 rounded-lg object-cover" alt="">
                        @else
                            <div class="h-8 w-8 sm:h-10 sm:w-10 rounded-lg bg-slate-100 flex items-center justify-center">
                    ...
                    <td class="px-2 sm:px-5 py-2 sm:py-3 max-w-0">
                        ...
                        <p class="text-[10px] sm:text-xs text-slate-400 mt-0.5 truncate">/artikel/{{ $article->slug }}</p>
                    </td>
                ...
                <tr>
                    <td colspan="4" class="px-5 py-10 sm:py-12 text-center text-xs sm:text-sm text-slate-400">
                        Belum ada artikel. <a href="{{ route('admin.articles.create') }}" class="text-sky-600 hover:underline">Tambah sekarang</a>
                    </td>
                </tr>
                @endforelse
            </tbody>
    ...
    <div class="border-t border-slate-100 px-3 sm:px-5 py-3">{{ $articles->links() }}</div>
    @endif
</div>
